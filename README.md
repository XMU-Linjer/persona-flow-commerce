...
V1.2 处于 planned 状态，规划方向是“异步 Profile Agent 工作流与 Artifact 追踪”：把 V1.1 的同步画像刷新升级为可追踪的 workflow，通过 RabbitMQ Agent Bus 传递 AgentMessage，并保留每个 Agent 的结构化 Artifact 与 Critic 审核记录。
...
### V1.2 planned：异步 Profile Agent 工作流与 Artifact 追踪

V1.2 计划在 V1.1 的行为画像闭环之上继续增强工程可解释性：

- Java 创建 `agent_workflow`；
- Java 通过 `commerce.agent.exchange` 投递 `AgentMessage`；
- Python Profile Manager 编排 Behavior / Intent / Trend / Critic 等规则 Agent；
- 每个 Agent 产出结构化 Artifact；
- Java 保存 workflow / task / artifact；
- workflow 成功后保存 `user_profile_version`；
- 前端展示 workflow 状态、Agent 执行步骤和 Artifact 留痕；
- Critic 审核 `PAYMENT_SUCCESS` 是否被正确解释为“偏好确认 + 当前需求满足 + 互补需求触发”。

V1.2 不是已完成能力，也不计划接入真实 LLM、RAG、复杂推荐算法、真实支付、分布式事务或完整 Outbox。Admin Lite 可以作为后期展示增强，但不是 V1.2 主线。
...
- 执行 `docker compose up -d`，启动 MySQL / Redis / RabbitMQ；
- 在独立 PowerShell 窗口启动 Python Agent；
- 在独立 PowerShell 窗口启动 Java 后端；
- 在独立 PowerShell 窗口启动 Vue 前端，默认绑定 `http://127.0.0.1:5173/`；
- 输出常用访问地址。
...
`stop-dev.ps1` 默认执行 `docker compose stop`，不会执行 `docker compose down -v`，不会删除数据库 volume。

`check-dev.ps1` 会自动检查 Vue 的 5173-5176 端口，并同时尝试 `127.0.0.1`、`localhost` 和 `[::1]`，避免 Vite 只监听 IPv6 localhost 时误判。
...
如果 5173 被占用，Vite 可能自动切换端口，请以前端终端输出的 Local URL 为准。
