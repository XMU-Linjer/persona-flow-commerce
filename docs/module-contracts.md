...
跨模块使用专门的请求模型和返回模型
业务模块通过 BehaviorEventPublisher 发布行为事件
```
...
这些数据将复制到 `order_item` 中，防止商品后续改名、改价或下架影响历史订单。
...
不得实现封号、角色管理、邮箱、手机号、OAuth 或 Refresh Token
不得让 admin 修改账户
发现约定冲突或缺失时停止实现并报告
只实现当前任务允许的文件范围
...
未来 catalog、shopping、trade 和 behavior 开工前，再逐步补充对应的数据库、HTTP 和事件约定。
...
```txt
把更新后的 module-contracts.md 和 01-account.md 放入仓库
检查两份文档没有冲突
生成 account 模块 Codex 任务
让 Codex 只实现 auth 与 user
运行迁移、测试和前后端接口验证
实现完成后把 account 状态改为“已实现”
```

其他模块文档不需要在 account 开工前全部完成。
