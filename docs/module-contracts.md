...
| trade | shopping | 后续如支持从购物车结算，可读取并清理购物车项；V1.0 非强依赖 |
...
catalog 只提供商品快照
shopping V1.0 不提供购物车结算项；如后续支持从购物车结算，再通过 shopping API 提供
behavior 事件在 behavior API 确认后才接入，并在业务成功后发布
```
...
catalog 的 ProductQueryApi 与 ProductSnapshot 已在本文件和 `modules/02-catalog.md` 中确定。shopping 的收藏和购物车 HTTP 范围已在 `modules/03-shopping.md` 中确定，且 V1.0 不提供 CartQueryApi、CartCheckoutApi 或 CartItemSnapshot。未来 trade 和 behavior 开工前，再逐步补充对应的数据库、HTTP 和事件约定。
...
```txt
shopping 收尾后进入 trade 设计与冲突收口
trade 开工前需要确认订单、库存、支付状态、购物车结算是否接入以及 behavior 事件边界
未确认前不新增 CartQueryApi、CartCheckoutApi 或 CartItemSnapshot
```

trade 和 behavior 文档在对应模块开工前继续收口。
