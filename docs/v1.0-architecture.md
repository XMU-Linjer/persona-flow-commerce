...
account 的对外能力已经在 `module-contracts.md` 和 `modules/01-account.md` 中确定。其他模块能力仍是待后续模块设计确认的草案。
...
| catalog | 查询商品详情 | shopping、trade、Agent | 草案 |
| catalog | 校验可购买 SKU | shopping、trade | 草案 |
| catalog | 获取订单商品快照 | trade | 草案 |
| shopping | 获取待结算购物车项 | trade | 草案 |
| shopping | 移除已购买购物车项 | trade | 草案 |
| trade | 查询订单摘要 | admin、Agent | 草案 |
...
不得实现封号、角色管理、邮箱、手机号、OAuth、Refresh Token 或 Token 黑名单。
不得让 admin 修改账户。
...
catalog、shopping、trade、behavior 的跨模块 Java 方法签名
