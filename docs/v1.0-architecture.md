...
- 后续如支持从购物车结算，再通过 shopping 获取并清理购物车项；
...
后续如支持从购物车结算，trade 可调用 shopping.api
...
| `shopping_cart_item` | shopping | 仅 shopping 直接读写；trade 后续如支持从购物车结算再通过 shopping API |
...
sequenceDiagram
    ...
    Inventory->>DB: 条件更新库存
    Order->>DB: 保存订单与订单项
    Note over Order,Catalog: 从购物车结算与清理购物车项留到 trade 设计阶段确认
    Order-->>Web: 返回待支付订单
