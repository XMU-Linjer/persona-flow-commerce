...
| shopping | CartCheckoutApi / CartItemSnapshot | trade | V1.0 暂不提供，后续如支持从购物车结算再确认 |
...
sequenceDiagram
    ...
    Note over Order,Shopping: V1.0 trade 可直接接收 skuId + quantity 创建订单
    Order->>Catalog: 获取商品和 SKU 快照
    Order->>Inventory: 扣减库存
    Inventory->>DB: 条件更新库存
    Order->>DB: 保存订单与订单项
    Note over Order,Shopping: 从购物车结算与清理购物车项留到 trade 设计阶段确认
    Order-->>Web: 返回待支付订单
