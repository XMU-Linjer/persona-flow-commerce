...
V1.2 planned 会在当前规则版能力之上升级为异步 workflow，并记录 workflow / task / Artifact。详细设计见 [异步 Profile Agent 工作流](02-async-profile-agent-workflow.md)。
...
V1.2 planned 中，`commerce.agent.exchange` 将正式用于异步 Profile Agent workflow：Profile Manager 通过 AgentMessage 编排 Behavior / Intent / Trend / Profile Builder/Critic，各 Agent 产出结构化 Artifact，Java 保存 workflow / task / artifact，并在 workflow 成功后保存 `user_profile_version`。
...
下一阶段 V1.2 计划强化的是异步执行和过程可追踪，而不是引入真实 LLM 或复杂推荐算法。
